...
const Card = ({ card, index }) => {
    ...
    return (
        <motion.div
            ...
            className={`group relative overflow-hidden rounded-3xl bg-neutral-900 ${spanClasses} ${isPenthouse ? 'border border-travel-white/20 shadow-[0_0_30px_-5px_rgba(113,113,122,0.3)]' : ''}`}
        >
            {/* Video Background */}
            ...
            <div className="absolute inset-0 z-20 flex flex-col justify-between p-6 md:p-8 transition-transform duration-700 ease-[0.25,0.1,0.25,1.0] group-hover:-translate-y-2">
                ...
                <div className="flex justify-between items-start">
                    <span className={`inline-block rounded-full backdrop-blur-md px-3 py-1 font-mono text-xs border transition-colors duration-500 ${isPenthouse ? 'bg-travel-white/10 text-travel-white border-travel-white/30' : 'bg-black/30 text-white/80 border-white/10'}`}>
                        {card.stats}
                    </span>
                    <div className={`opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100 bg-white text-black rounded-full p-2 ${isPenthouse ? 'bg-travel-white text-black' : ''}`}>
                ...
                {isPenthouse && (
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center opacity-80 group-hover:opacity-100 transition-opacity duration-700 ease-in-out">
                        <div className="w-12 h-1 bg-travel-white/50 rounded-full mb-2 shadow-[0_0_10px_rgba(255,255,255,0.5)]" />
                        <div className="w-12 h-1 bg-travel-white/30 rounded-full" />
                    </div>
                )}

                {/* Bottom: Info */}
                <div className="translate-y-4 transition-transform duration-700 ease-[0.25,0.1,0.25,1.0] group-hover:translate-y-0">
                    <p className={`font-mono text-xs ${isPenthouse ? 'text-travel-white' : 'text-travel-accent'} mb-2 uppercase tracking-wide`}>
                        {card.subtitle}
                    </p>
                    ...
                    <div className="h-0 opacity-0 overflow-hidden group-hover:h-auto group-hover:opacity-100 transition-all duration-700 ease-in-out">
                        ...
                        <button className={`mt-4 text-xs font-bold uppercase tracking-widest text-white border-b hover:text-travel-white hover:border-travel-white transition-colors duration-300 ${isPenthouse ? 'border-travel-white pb-1 text-travel-white' : 'border-travel-white pb-1'}`}>
                            Explore
                        </button>
                    </div>
                </div>
            ...
            {isPenthouse && (
                <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-transparent via-travel-white to-transparent opacity-50" />
            )}
...
export default function BentoGrid() {
    return (
        <section className="px-4 py-24 pb-60 max-w-7xl mx-auto bg-[#0c0c0c]">
            <div className="mb-16 md:mb-24 text-center">
                <h2 className="font-serif text-5xl md:text-7xl text-white mb-6 tracking-tight">
                    Curated Lifestyle
                </h2>
                <p className="text-white/60 text-center max-w-2xl mx-auto font-sans text-lg">
                    Experience the zenith of luxury through our curated selection of high-tech environments and elite memberships.
                </p>
            </div>
